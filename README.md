...
공진형 컨버터는 제어 방식에 따라 주파수를 바꾸는 PFM(Pulse Frequency Modulation)과 듀티를 바꾸는 PWM(Pulse Width Modulation) 방식으로 해석할 수 있습니다. 기존에는 PFM 기반 SRC 소신호 모델링 연구가 많이 다뤄졌지만, 이 논문은 APWM 기반 SRC의 듀티 변화가 출력 응답에 미치는 영향을 모델링합니다.

일반적인 상태공간 평균화는 한 주기 평균값을 중심으로 모델을 만들기 때문에, 공진 탱크의 교류 성분을 충분히 표현하기 어렵습니다. 논문에서는 상태공간 평균화를 그대로 적용하면 공진회로의 `L-C` 동특성이 평균화 과정에서 사라지는 문제가 있다고 보고, 이를 보완하기 위해 EDF(Extended Describing Function)를 사용합니다.
...
PFM은 스위칭 주파수를 조절해 공진점과의 거리를 바꾸는 방식이고, PWM은 스위칭 주파수는 두고 펄스 폭 또는 듀티를 조절하는 방식입니다. APWM은 PWM의 한 형태로, 풀브리지 양쪽 스위칭 신호를 비대칭으로 구동해 브리지 출력전압 `VAB`의 듀티를 조절합니다.

APWM에서 듀티가 변하면 `VAB`의 기본파 성분과 DC 성분이 함께 변하고, 이 변화가 공진 탱크 전류와 출력전압 응답에 영향을 줍니다.
...
| 상태공간 평균화 한계 | 일반 평균화만 적용하면 공진 탱크의 `L-C` 교류 동특성이 사라져 SRC의 주파수 응답을 제대로 표현하기 어려움 |
